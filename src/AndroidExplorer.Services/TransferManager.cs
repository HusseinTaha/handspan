...
public sealed class AdbSizeMismatchException(long expected, long actual)
    : DeviceException(
        "The transfer finished with the wrong size, so it was not kept. Android Explorer will retry.",
...
public sealed class AdbHashMismatchException()
    : DeviceException(
        "The transferred file did not match the original, so it was not kept. Android Explorer will retry.",
        "sha-256 mismatch")
{
    public override bool IsTransient => true;
}
